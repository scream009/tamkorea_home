...
const AdSettings = ({ ad, upside }) => {
  ...
  const NUDGE = {
    ...
    no_balance: {
      ...
      body: <>잔액이 없어 노출 실적이 발생하지 않았고, 아래 집행액과 소진률이
...
const CpcSection = ({ cpc, funnel, dominance, store, adSet, projection }) => {
  ...
  if (bal <= 0 && yst > 0) {
    // 잔액 0 + 어제 소진 있음 = **방금 바닥난 것**. 광고 공백이 막 시작된 시점이라
    // 가장 급하다. (한때 '대행 계정 집행'으로 봤으나 근거가 없었다 — 2026-08-13
    // FK 통보액과 수집값이 소수점까지 일치했고 일예산×일수로 정확히 소진됐다.)
    lvl = 'urgent'; tag = '방금 소진'; bs = '어제 집행을 끝으로 잔액 소진';
    recomTitle = '🔴 잔액이 막 소진됐습니다 — 지금 충전하면 공백이 없습니다';
    recomBody = (
      <>어제 <b>{r0(yst)}元</b> 집행을 끝으로 잔액이 바닥났습니다. 지금 충전하시면 노출이 끊기지 않고 이어집니다. 권장 충전액 <b>일예산×7일</b> 기준입니다.</>
    );
    contrib = (
      <>📉 <b>지금이 분기점입니다</b> — 어제까지는 <b>{r0(yst)}元</b>이 정상 집행돼 노출을 견인했지만, 오늘부터는 광고 노출이 멈춥니다. 상권 노출 순위는 하락에 시간이 걸리지 않습니다.</>
    );
  } else if (bal <= 0) {
