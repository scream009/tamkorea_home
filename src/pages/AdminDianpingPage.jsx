...
export default function AdminDianpingPage() {
  ...
  const askRefresh = useCallback(async (r, mode = 'get') => {
    ...
    if (mode !== 'get') {
      ...
      // 🔴 2026-09-23 (Owner): 워커의 AUTO_APPROVE 가 켜져 사람 승인 단계가 없어졌다.
      //    누르면 **그대로 고객사 톡방으로 나간다.** 여기가 마지막 확인이다.
      //    예전 문구('바로 나가지 않습니다')를 그대로 두면 담당자가 안 나갈 줄 알고 누른다.
      //    워커를 AUTO_APPROVE=False 로 되돌리면 이 문구도 같이 되돌려야 한다.
      if (!window.confirm(`${r.name}\n${what} 고객사 톡방으로 바로 발송합니다.\n`
        + '⚠️ 승인 단계가 없습니다 — 누르면 그대로 나갑니다. 계속할까요?')) return;
    }
