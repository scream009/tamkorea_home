...
// ─── 유형별 업로드 마감 일수 결정 ──────────────────────────────────────────
// 체험단(체험 계열) = 촬영일 +7일 / 인플·기자단 = 촬영일 +14일(2주)
//
// ⚠️ 체험 계열 7일은 담당자 진도보드(staff-board.js·staff-infl.js 의 UPLOAD_GRACE_DAYS)와
//    같은 값이어야 한다. 예전에는 여기가 5일이라, 인플루언서에게 안내한 마감(5일)과
//    우리가 지연으로 세는 시점(7일) 사이 이틀이 아무도 책임지지 않는 구간이었다.
//    (Owner 결정 2026-08-24 — 업계 표준이 방문 후 7~14일이라 7일로 통일)
function deadlineDaysForType(type) {
  const t = (type || '').trim();
  if (t === '체험' || t === '체험단' || t === '기자→체험') return 7;
