    ...
    await tk('예약입력_DB', {
      ...
      body: JSON.stringify({
        records: [{ fields: {
          ...
          대표인플: [inflId],
          'XHS_ID_': [inflId],
          // 🔴 인원메모는 **고객사에게 그대로 나간다** — 예약메시지 수식이 "인원: N명 (인원메모)"
          //    로 끌어 쓴다. 내부 메모를 여기 넣으면 고객사 카톡에 그대로 찍힌다
          //    (실제로 '모집사이트 선발 · 희망 14:30 · 위챗 -' 이 나갔다, Owner 2026-08-15).
          //    → 내부 기록은 전부 비고로. 인원메모는 고객사가 알아야 할 것만 쓴다.
          비고: `${tag} · 담당 ${mgr} · 모집사이트 선발`
            + `${hasTime ? ` · 희망 ${hhmm}` : ' · 시간 미기재(기본 12:00)'}`
            + ` · 위챗 ${a['🔒 pii_wechat'] || '-'}`
            + `${inflNew ? (a.xhs_followers ? ' · 인플 신규등록(PAL 자기신고)' : ' · 인플 신규등록(PAL 미검증)') : ''}`
            + ' · 계약월 확인 필요',
        } }],
        typecast: true,
      }),
